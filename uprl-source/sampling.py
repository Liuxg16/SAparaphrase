...
def simulatedAnnealing_batch(config, dataclass):
    option = config
    if option.mcmc=='sa': 
        agent = UPRL(config)
    ...
    for i in range(1000000):
        sen_id = i% use_data.length
        sta_vec=sta_vec_list[sen_id*batch_size:sen_id*batch_size+batch_size][0]
        input, sequence_length, _=use_data(batch_size, sen_id)
        assert len(input)==len(sequence_length)
        N_input = len(input)
        loss, rewards = agent(input, sta_vec+[0], id2sen, emb_word)
        loss.backward()
        torch.nn.utils.clip_grad_norm(agent.parameters(), 5)
        optimizer.step()

        print('sentences: {}, avg reward {}, loss {}'.format(i, rewards.item(),loss.item()))
